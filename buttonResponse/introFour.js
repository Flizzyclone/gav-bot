...
module.exports = {
	...
	async execute(interaction, client) {
        ...
        let roleName = interaction.customId.replace("introFour_","");
        let roleId = config.intro.roles.romantic[roleName];
        let role = await guild.roles.fetch(roleId);
        if(member.manageable) {
            member.roles.add(role);
        }

        intro.updateUserValue('romantic',roleId,interaction.user.id);
        intro.stageFive(member);
